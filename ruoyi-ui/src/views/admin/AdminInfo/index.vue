    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['admin:AdminInfo:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['admin:AdminInfo:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['admin:AdminInfo:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['admin:AdminInfo:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="AdminInfoList" @selection-change="handleSelectionChange">
...
<script>
import { listAdminInfo, getAdminInfo, delAdminInfo, addAdminInfo, updateAdminInfo } from "@/api/admin/AdminInfo";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listAdminInfo(this.queryParams).then(response => {
        this.AdminInfoList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getAdminInfo(adminId).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.adminId != null) {
            ...
            addAdminInfo(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除管理员信息编号为"' + adminIds + '"的数据项？').then(function() {
        return delAdminInfo(adminIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('admin/AdminInfo/export', {
        ...this.queryParams
      }, `AdminInfo_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
